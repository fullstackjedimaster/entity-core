...
#   - derives tenant/schema/org information from claims,
#   - calls ec-model over HTTP with a RequestEnvelope and the caller's JWT.
# ---------------------------------------------------------------------------

# Name of the claim in the JWT that holds the tenant/schema key.
SCHEMA_CLAIM = env("EC_SCHEMA_CLAIM") or "schema"

# Default schema to use if the claim is missing.
...
def get_effective_schema(claims: Dict[str, Any]) -> str:
    ...
    logical schema / tenant key to pass down to ec-model.
