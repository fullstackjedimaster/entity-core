...
APP_ADMIN_DATABASE_URL="postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${APP_POSTGRES_DB}?sslmode=disable"

log "Using APP_ADMIN_DATABASE_URL=$APP_ADMIN_DATABASE_URL"
log "Using SQL_FILE=$SQL_FILE"

psql "$APP_ADMIN_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$SQL_FILE"
